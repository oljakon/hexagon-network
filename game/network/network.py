...
class Network:
    ...
    def __init__(self,
                 sig: pyqtSignal,
                 sig_move_army: pyqtSignal,
                 sig_chg_move: pyqtSignal,
                 sig_add_podsvet: pyqtSignal,
                 sig_delete_podsvet: pyqtSignal,
                 sig_enter_ok: pyqtSignal):
        ...
        self.sig_enter_ok = sig_enter_ok
        ...
        self.player = dict_get[0]['player']
        self.session = dict_get[0]['session']
    ...
    def wait_server(self):
        ...
        data = self.s.recv(1000000)
        print(len(data))
        dict_get_list = HexagonProtocol.getDataFromByteStr(data)
        for dict_get in dict_get_list:
            ...
            elif dict_get['type'] == 'buy_army':
                self.sig_enter_ok.emit(dict_get['data'])
    ...
    def buyArmy(self, hiring, entered_cell, mas_label):
        send_mes_dict = {'session': self.session, 'player': self.player, 'type': 'buy_army', 'data': [hiring, entered_cell, mas_label]}
        send_mes_bit = HexagonProtocol.getByteStrFromData(send_mes_dict)
        self.s.send(send_mes_bit)